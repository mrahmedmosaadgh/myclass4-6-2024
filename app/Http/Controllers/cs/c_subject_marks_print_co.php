...
class c_subject_marks_print_co extends Controller
{
  ...
  return response()->json(['msg' => 'year_semesters successfully loaded.', 
...
$data['mark']==null;
   ...
   $sum= c_mark::where('school_id',$data['school_id'])->
    where('semester_id',$data['semester_id'])->
    where('student_id',$data['student_id'])->
    where('subject_id',$data['subject_id'])
    ->sum('mark');
  $letter=  $this->is_between($sum) ;
  $c_mark_sum= c_mark_sum::where('school_id',$data['school_id'])->
    where('semester_id',$data['semester_id'])->
    where('student_id',$data['student_id'])->
    where('subject_id',$data['subject_id'])->update([
            "mark"=> $sum,
            "letter"=> $letter,
    ]);
return  c_mark_sum::where('school_id',$data['school_id'])->
    where('semester_id',$data['semester_id'])->
    where('student_id',$data['student_id'])->
...
// 95
    // return 'admin_save_mark done' ;
        ...
        if ($fun == 'show_all') {
            ...
            foreach ($c_student as $key => $student) {
                ...
                foreach ($c_mark as $key => $mark) {
                    ...
                    $mark['my_subject_titles'] = $c_subject_title;
        ...
        if ($fun == 'show') {
            ...
            $semester_id =null;
            if(isset($request->data['data']['semester_id'])){
